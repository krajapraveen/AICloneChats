    ...
    <div className="min-h-screen bg-cream">
      ...
      <div className="max-w-6xl mx-auto px-5 md:px-8 py-10 md:py-14" data-testid="dashboard-page">
        <div className="flex items-end justify-between gap-4 flex-wrap mb-10">
          <div>
            <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground mb-2">CLONE HQ</p>
            <h1 className="heading-display text-4xl md:text-5xl">Hey {user.name?.split(" ")[0] || "you"}.</h1>
            <p className="mt-2 font-medium text-muted-foreground">Your AI selves, all in one place.</p>
          </div>
        ...
        {loading ? (
          <p className="font-display">Loading your clones…</p>
        ) : clones.length === 0 ? (
          <div className="brutal-card p-10 text-center bg-lemon" data-testid="empty-state">
            <h2 className="heading-display text-3xl mb-2">No clones yet.</h2>
            <p className="text-foreground/80 font-medium mb-6">Build your first AI version. Takes 3 minutes.</p>
            <Link to="/clones/new" className="btn-brutal bg-white" data-testid="empty-create-btn">Create your clone</Link>
          </div>
        ) : (
            ...
            {clones.map((c) => (
              <div key={c.clone_id} className="brutal-card p-6 flex flex-col" data-testid={`clone-card-${c.slug}`}>
                <div className="flex items-start gap-3 mb-4">
                  {c.avatar_url ? (
                    <img src={c.avatar_url.startsWith("/") ? `${process.env.REACT_APP_BACKEND_URL}${c.avatar_url}` : c.avatar_url} alt={c.display_name} className="w-14 h-14 rounded-full border-2 border-ink object-cover" />
                  ) : (
                    <div className="w-14 h-14 rounded-full border-2 border-ink bg-lilac flex items-center justify-center font-display font-black text-xl">
                      {c.display_name?.[0]?.toUpperCase() || "C"}
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <h3 className="font-display font-bold text-xl truncate">{c.display_name}</h3>
                    <p className="font-mono text-xs text-muted-foreground truncate">/{c.slug}</p>
                  </div>
                  <span className={`tag ${c.visibility === "public" ? "bg-mint" : c.visibility === "private" ? "bg-bubblegum" : "bg-lilac"}`}>
                    {c.visibility}
                  </span>
                </div>

                {c.bio && <p className="text-sm font-medium text-foreground/80 mb-4 line-clamp-2">{c.bio}</p>}
